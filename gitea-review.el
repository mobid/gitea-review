...
;; Package-Requires: ((emacs "29.1") (forge "20231213") (ghub "20220621") (deferred "20170901"))
...
;;
;; For the time when review is in progress, `emacs-lock-mode' is
;; active for review buffer.
;;
;;
...
This function is deferred."
  (interactive (list (cond
                      ((derived-mode-p 'forge-pullreq-mode) (oref forge-buffer-topic number))
                      (t (let ((pullreq (forge-get-pullreq
                                         (forge-read-pullreq "Pullreq:"))))
                           (oref pullreq number))))))
...
OLD-LINE take precedence over NEW-LINE, only one should be given."
  (let* ((old? old-line)
    ...
    (goto-char (point-min))
    (unless (re-search-forward (concat "\\(---\\|\\+\\+\\+\\) \\(a\\|b\\)/" file) nil t)
      (error "Hunk file not found: %s" file))
...
(defun gitea-review--place-comment (comment)
  ...
  (let ((inhibit-read-only t)
    ...
    (unless (ignore-errors
              ...
              t)
      ;; Hunk of this comment was proparbly deleted.  Comment will be
      ;; placed at end of file.
      ;;
      ;; NOTE: Comments has attached diff info, we could show it in
      ;; comment.
      (setq trashed-comment t)
      (goto-char (point-max)))
    (forward-line 1)
    (while (looking-at "# ")
      (forward-line))
    ...
    (let ((ago (apply #'format "%s %s ago"
      ...
      (insert (propertize (format "# %s %s%s\n" (gitea-comment-author comment) ago
                                  (cond
                                   ((gitea-comment-new comment) " ⭐")
                                   (trashed-comment " (missing hunk)")
                                   (t "")))
...
(defun gitea-review--goto-change-line ()
